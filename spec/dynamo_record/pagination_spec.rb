...
RSpec.describe DynamoRecord::Pagination do
  let(:items) do
    5.times.map do |i|
  ...
  describe '.paginate_array' do
    it 'returns first page of items' do
      result = DynamoRecord::Pagination.paginate_array(items, nil, per_page: 3)
    ...
    it 'returns all items when per_page exceeds count' do
      result = DynamoRecord::Pagination.paginate_array(items, nil, per_page: 10)
    ...
    it 'paginates with cursor' do
      first_page = DynamoRecord::Pagination.paginate_array(items, nil, per_page: 2)
      second_page = DynamoRecord::Pagination.paginate_array(items, first_page.next_cursor, per_page: 2)
    ...
    it 'clamps per_page to MAX_PER_PAGE' do
      result = DynamoRecord::Pagination.paginate_array(items, nil, per_page: 999)
      expect(result.per_page).to eq(100)
    end

    it 'clamps per_page minimum to 1' do
      result = DynamoRecord::Pagination.paginate_array(items, nil, per_page: 0)
      expect(result.per_page).to eq(1)
    end
  end

  describe DynamoRecord::Pagination::PaginatedResult do
    it 'is enumerable' do
      result = DynamoRecord::Pagination::PaginatedResult.new(items: items, next_cursor: nil, per_page: 25)
      expect(result.map(&:id)).to eq(items.map(&:id))
    end

    it 'provides pagination_metadata' do
      result = DynamoRecord::Pagination::PaginatedResult.new(items: items, next_cursor: 'abc', per_page: 5)
